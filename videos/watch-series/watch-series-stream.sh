...
mpv "$(wget -q -O- "$d" |grep -i data-video|cut -d\" -f6|grep '^http'|sort -r|grep -v openload)"
if [ "$?" == 2 ]
then 
  mpv "$(wget -q -O- "$d" |grep -i data-video|cut -d\" -f6|grep '^http'|sort -r|grep -v openload|head -n1)"
fi
